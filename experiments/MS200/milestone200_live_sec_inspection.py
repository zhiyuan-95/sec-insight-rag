"""Manual examination harness for the Plan 200 company ingestion workflow."""
...
def main(argv: Sequence[str] | None = None) -> int:
    """Run the Milestone 200 experiment and write inspection artifacts."""
...
def _parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate a manual examination report for the Plan 200 ingestion workflow.",
    )
    ...
    parser.add_argument(
        "--full-report",
        action="store_true",
        help="Accepted for compatibility; the saved report keeps the fixed Plan 200 mapping shape.",
    )
...
def format_lineage_report(run: ExperimentRun) -> str:
    ...
    lines = [
        "Milestone 200 Financial Metric Data Lineage View",
        "",
        "Run Context:",
        f"  ticker: {run.ticker}",
...
def format_report(run: ExperimentRun, *, report_output: str = "file") -> str:
    ...
    lines = [
        "# Milestone 200 Live SEC Experiment Report",
        "",
        "## Run Context",
        "",
    ]
...
def format_saved_report(run: ExperimentRun, *, full_report: bool = False) -> str:
    """Render the saved Plan 200 target mapping report."""
    lines = [
        "# Plan 200 Target Mapping Report",
    ]
...
def _saved_compact_summary_rows(run: ExperimentRun, *, full_report: bool) -> list[dict[str, Any]]:
    ...
    report_output = "saved Plan 200 target mapping report"
    if full_report:
        report_output = "saved Plan 200 target mapping report; --full-report kept for CLI compatibility"
...
def format_compact_report(run: ExperimentRun, *, full_report: bool = False) -> str:
    ...
    report_output = (
        "saved Plan 200 target mapping report; --full-report compatibility flag accepted"
        if full_report
        else "saved Plan 200 target mapping report"
    )
    lines = [
        "Milestone 200 Plan 200 Ingestion Examination",
        "",
        "Run Context",
        f"  ticker: {run.ticker}",
        f"  run timestamp: {run.run_timestamp}",
    ...
    lines.extend(
        [
            "",
            "Saved Report",
            "  milestone200_mapping_report contains compact summary, target metric status, and 10-K/10-Q proposed formulas.",
        ]
    )
    return "\n".join(lines)
